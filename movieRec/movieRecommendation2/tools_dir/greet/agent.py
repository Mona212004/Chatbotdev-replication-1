...
from ...entities.user2 import UserFunctions, User, MoviePreferences
from ...test_models import greeting_model

from google.adk.agents import LlmAgent
from google.adk.models.lite_llm import LiteLlm
from .greet_prompt5 import greeting_instructions
...
def sayHello(username: str, user_id: Optional[Any] = None) -> str:
    ...
    if user_id is None:
        ...
        if User is None:
            ...
            return f"AUTH_SUCCESS: User {User.user_name}! Welcome to MovieRec! We have created a new account for you, your User ID is {User.user_id}. Please remember your user ID. I am handing you over to the recommendation expert now."
        else:
        ...
        return f"AUTH_SUCCESS: User {new_user.user_name}! A new account has been created for you. Your new User ID is {new_user.user_id}. Please remember your user ID. I am handing you over to the recommendation expert now."
    else:
        ...
        return f"AUTH_SUCCESS: User {User.user_name}, User ID: {User.user_id}! Your preferences are {User.preferences}. I am handing you over to the recommendation expert now!"
...
# greeting agent will need to call sayHello multiple times with different args to handle the conversation flow for greeting and onboarding users based on whether they are new or returning, and whether they can provide their user ID or not. The tool is designed to manage user accounts effectively while providing a personalized greeting experience.
greeting_agent = None
try:
    greeting_agent = LlmAgent(
    model=LiteLlm(model=greeting_model, api_base="http://localhost:11434", tool_choice="auto", temperature=0),
    name="greeting_agent",
    description="You are a greeting agent that greets users and manages their accounts in the database.",
    instruction=greeting_instructions,
    tools=[sayHello],
    output_key="greeting_response",
    )
except Exception as e:
    print(f"Error initializing greeting agent: {e}")
