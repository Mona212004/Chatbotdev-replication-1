...
async def preference_management_instructions(context: ReadonlyContext) -> str:
    ...
    Locate the message containing "AUTH_SUCCESS" and extract the numeric User ID from it.
    It may appear in these formats:
    - "AUTH_SUCCESS: User Alex, User ID: 51!" → user_id = 51
    - "AUTH_SUCCESS: User Alex, User ID: 51" → user_id = 51
    Extract ONLY the integer that appears after "User ID:" in that message.
    You MUST use this exact numeric user_id when calling any tool. Never guess or assume it. If you cannot find "User ID:" in the history, stop and output: "Error: could not find user ID. Please log in again."
    ...
    ### EXECUTION STEPS
    1. **Extract User ID:** Find the numeric User ID from the "AUTH_SUCCESS" message (e.g., "User ID: 51" → user_id=51).
    2. **Classify Intent and call the correct tool:**
        ...
        **Case E — Movie title interest (Explicit, Implied, or Mixed with genres):**
        ...
        User says: "i like crime movies like 'the godfather'"
        → call update_preferences(user_id, liked_genres=["crime"], movie_interests_titles=["the godfather"])
        User says: "i enjoy horror films like 'it' and 'the shining'"
        → call update_preferences(user_id, liked_genres=["horror"], movie_interests_titles=["it", "the shining"])
        ...
        **Case H — Remove some AND add some in the same message:**
        User says: "i do not like horror anymore, now i like comedy", "remove romance, add action", "i don't like la la land and the godfather, now i like harry potter and fantasy"
        Removal signals: "do not like", "don't like", "no longer like", "anymore", "remove", "dislike", "hate"
        Addition signals: "now i like", "i like", "i enjoy", "add", "i love", "i'm into", "movies like"

        Example 1: "i do not like 'la la land' and 'the godfather' anymore. I also do not like 'romance' and 'crime'. Now i like 'harry potter' and fantasy."
        → remove_preferences(user_id, liked_genres=["romance", "crime"], movie_interests_titles=["la la land", "the godfather"])
        → update_preferences(user_id, liked_genres=["fantasy"], movie_interests_titles=["harry potter"])

        Example 2: "i do not like harry potter anymore. I also do not like crime, fantasy and comedy. I like romance movies like 'la la land'."
        → remove_preferences(user_id, liked_genres=["crime", "fantasy", "comedy"], movie_interests_titles=["harry potter"])
        → update_preferences(user_id, liked_genres=["romance"], movie_interests_titles=["la la land"])

        1. Collect ALL genres and titles paired with removal signals into one single remove call.
        2. Wait for the response.
        3. Collect ALL genres and titles paired with addition signals into one single update call.
        4. Wait for the response.
        Use [] for whichever field has no items in each call.
    ...
    - When the user message contains BOTH removals AND additions in the same message: make exactly 2 tool calls in order — remove first, then add.
